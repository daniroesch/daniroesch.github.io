...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function buildBook(imageUrls, folder, width, height, initialPage = 0) {
        ...
        try {
            ...
            loadingScreen.innerHTML = `<div class="menu-wrapper"><div class="menu-row"><span class="bracket">[</span><span class="menu-links">Fehler beim Laden</span><span class="bracket">]</span></div></div>`;
            return;
        }
    ...
    document.addEventListener('click', (e) => {
        // =====================================================================
        // 🔥 NEU: Die absolute Lösung für die Klick-Animation (Race Condition)!
        // =====================================================================
        if (e.target.closest('#back-to-start-btn') || e.target.closest('#home-btn')) {
            e.preventDefault();
            if (pageFlip && !isZoomed()) {
                
                // 1. Zwinge das Menü sofort in den Hintergrund (z-index 1) 
                // und schalte den Start-Text ohne Verzögerung ein.
                menuPositioner.style.zIndex = '1';
                startMenu.style.transition = 'none';
                startMenu.style.opacity = '1';
                endOfBookMenu.style.transition = 'none';
                ...
                // 2. Ein "unsichtbarer" Abruf zwingt den Browser dazu,
                // diese Befehle JETZT SOFORT auszuführen und auf den Bildschirm zu zeichnen.
                void startMenu.offsetHeight; 

                // 3. Erst wenn der Text 100%ig fertig gezeichnet ist (nach 30 Millisekunden), 
                // darf die schwere Blätter-Animation des Buches starten. 
                setTimeout(() => {
                    pageFlip.flip(0);
                }, 30);
            }
